...
function generatePatternResponse(question, userData) {
    ...
    // Savings questions
    if (q.match(/save|saving|save more/)) {
        if (userData && userData.totalIncome && userData.totalExpenses) {
            ...
            return `You're currently saving ₹${currentSavings} per month (${savingsRate}% of income). ${topCategory ? `Your highest expense is ${topCategory.category} at ₹${topCategory.amount}. Consider reducing this by 10-15% to boost savings.` : 'To save more, track your top 3 spending categories and look for 10-15% reduction opportunities.'} The 50/30/20 rule suggests saving 20% of income, which would be ₹${Math.round(userData.totalIncome * 0.2)} for you.`;
        }
    ...
    // Budget/overspending questions
    if (q.match(/budget|overspend|spending too much/)) {
        if (userData && userData.budget) {
            ...
            return `Budget Health Check: You're using ${needsPercent}% of your Needs budget and ${wantsPercent}% of your Wants budget. ${needsPercent > 100 || wantsPercent > 100 ? '⚠️ You\'re overspending! Focus on cutting discretionary expenses (entertainment, dining out, shopping) by 20-30%.' : '✅ Good job staying within budget! Keep tracking consistently.'}`;
        }
    ...
    if (q.match(/should.*buy|purchase|afford/)) {
        if (userData && userData.budget) {
    ...
    // Income questions
    if (q.match(/income|earn|salary|make money/)) {
    ...
    // Debt questions
    if (q.match(/debt|loan|credit card|emi/)) {
    ...
    if (q.match(/invest|investment|mutual fund|stock|sip/)) {
    ...
    if (q.match(/emergency|emergency fund|rainy day/)) {
        if (userData && userData.totalExpenses) {
            const target = userData.totalExpenses * 6;
            return `Your emergency fund target should be ₹${target} (6 months of expenses). Build it in a separate savings account. Start by saving ₹${Math.round(target / 12)} per month for 12 months, or accelerate if possible.`;
        }
        return "Emergency fund = 6 months of living expenses. Keep it in a liquid, safe place (savings account or liquid fund). This protects you from job loss, medical emergencies, or unexpected expenses. Build it before investing.";
    }
